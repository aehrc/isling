...
def align_seqs(path, index, read1, read2, output, threshold, match, mismatch, h, threads):
	...
	#build bwa args
	bwa_args = [path, "mem", "-t", str(threads), "-A", str(match), "-B", str(mismatch), "-O", "6,6", "-L", "0,0", "-T", str(threshold), "-h", str(h), index, read1]
...
def main(argv):
	...
	#call to align_seqs
	align_seqs(args.bwa, args.index, args.read1, args.read2, args.output, args.threshold, args.match, args.mismatch, args.hflag, args.threads)
