...
if (!-e 'Trinity-init.fasta'){
    ...
    my $ILIBs = GetIdir();
    print "perl $ILIBs $trinity_script --seqType fa  --JM 4G --single blat_out_candidate_singlelane.fa --min_contig_length $min_contig_length  --output trinity_output --CPU $thread_no --bfly_opts \"-V 10 --stderr\"";
