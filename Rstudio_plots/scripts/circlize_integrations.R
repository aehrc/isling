...
data_path = "data/2019-06-17/"
out_path = "out/2019-06-17/"
files <- dir(data_path, pattern =".txt")

#import all datasets
data <- data_frame(filename = files) %>% # create a data frame holding the file names
  mutate(integrations = map(filename, ~ read_tsv(file.path(data_path, .), col_types = cols(Chr = col_character(), .default = col_guess()))) # a new data column
  ) 
...
for (i in datasets) {
  ...
  chroms <- if (str_detect(host, "hg38")) {paste0("chr", c(1:22, "X", "Y"))} 
  else {paste0("chr", c(1:20, "X", "Y"))}
