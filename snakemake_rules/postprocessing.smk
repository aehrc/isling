...
rule annotate_merged:
    input:
        ...
        annot = lambda wildcards: get_value_from_df(wildcards, 'annot_gff'),
    output:
        annotated = "{outpath}/{dset}/ints/{samp}.{host}.{virus}.integrations.post.unique.merged.annot.txt"
    container:
        "docker://szsctt/isling:latest"
    conda:
        "../envs/py37_pysam.yml"
    ...
    shell:
        """
        python3 scripts/annotate.py -i {input.merged} -g {input.annot} -o {output.annotated} 
        """

rule summarise:
    input: 
        merged_beds = lambda wildcards: expand(
                    strip_wildcard_constraints(rules.merged_bed.output.merged), 
                    zip,
                    samp = toDo.loc[toDo['dataset'] == wildcards.dset,'sample'],
                    host = toDo.loc[toDo['dataset'] == wildcards.dset,'host'],
                    virus = toDo.loc[toDo['dataset'] == wildcards.dset,'virus'],
