...
files <- dir(data_path, pattern ="integrations.txt", recursive=TRUE)
...
data <- data %>% 
  distinct(filename, Chr, IntStart, IntStop, VirusRef, VirusStart, VirusStop, NoAmbiguousBases, OverlapType, Orientation, PossibleHostTranslocation, PossibleVectorRearrangement, HostPossibleAmbiguous, ViralPossibleAmbiguous,
           .keep_all=TRUE)
